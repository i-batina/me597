...
class decision_maker(Node):
    ...
    def timerCallback(self):
        ...
        spin_once(self.localizer, timeout_sec=0.0)    # Remember that this file is already running the decision_maker node.
...
def main(args=None):
    ...
    if args.motion.lower() == "point":
        DM=decision_maker(Twist, "/cmd_vel", QoSProfile(depth=10), goalPoint=[-1.0, -1.0], motion_type=POINT_PLANNER)
    elif args.motion.lower() == "trajectory":
        DM=decision_maker(Twist, "/cmd_vel", QoSProfile(depth=10), goalPoint=[-1.0, -1.0], motion_type=TRAJECTORY_PLANNER)
    else:
